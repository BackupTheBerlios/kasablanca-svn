...
void Ftp::message(QString string) {

	messageBox(string, SlaveBase::Information, "kio-ftps debug message");
}
                          ...
                          int errorcode, KIO::fileoffset_t _offset )
{
  ...
  else
  {
	...
	if (!m_bPasv) {
		...
		m_portserver->waitForNewConnection(connectTimeout() * 1000);
		if (!m_portserver->hasPendingConnections()) message("The FTP server did not connect.");
		QTcpSocket *socket = m_portserver->nextPendingConnection();
		m_data = new QSslSocket();
		m_data->setSocketDescriptor(socket->socketDescriptor());
	}
